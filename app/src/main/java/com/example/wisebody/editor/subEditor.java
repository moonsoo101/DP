...
public class subEditor extends Activity implements View.OnClickListener {
    ...
    seqListAdapter seqRecylcerAdapter;
    ArrayList<seqData> seqLists = new ArrayList<>();
       ...
       @JavascriptInterface
        public void setSeqTime(final String start, final String end,final String id) { // must be final
            handler.post(new Runnable() {
                public void run() {
                    ...
                    seqLists.add(new seqData(start, end, id));
                    Collections.sort(seqLists);
                    int position = seqLists.indexOf(new seqData(start,end,id));
                    webView.loadUrl("javascript:drawSeq('" + start + "','" + end + "','" + id + "', '"+(position+1)+"');");
                    JSONArray jsonArray = new JSONArray();
                    JSONObject jsonobj;
                    for(int i =0;i<seqLists.size();i++) {
                        seqData item = seqLists.get(i);
                        try {
        ...
        final int position = index-1;
          handler.post(new Runnable() {
            ...
            public void run() {
               ...
               writeSubtitle.setText(seqLists.get(position).text);
                util = null;
            }
        });
        }
        ...
        public void updateRegion(final String start, final String end)
        {
            handler.post(new Runnable() {
                @Override
                public void run() {
                    seqData item = seqLists.get(seqRecylcerAdapter.selected_position);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        writeSubtitle.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if(seqRecylcerAdapter.selected_position!=-1) {
                    int position = seqRecylcerAdapter.selected_position;
                    seqListAdapter.ViewHolder viewHolder = (seqListAdapter.ViewHolder)seqRecylcer.findViewHolderForAdapterPosition(position);
                    viewHolder.seq_text.setText(writeSubtitle.getText().toString());
        ...
        seqRecylcerAdapter = new seqListAdapter(seqLists, R.layout.seq_item, preview, webView, writeSubtitle, pausebtn, seqRecylcer, secCont, editmodeBtnCont);
    ...
    public void onPause()
    {
        ...
        mgr.hideSoftInputFromInputMethod(writeSubtitle.getWindowToken(),0);
        super.onPause();
    }
    @Override
...
//                break;
            ...
            case R.id.prevBtn :
                if(seqRecylcerAdapter.selected_position!=-1) {
                    if (seqRecylcerAdapter.selected_position != 0) {
                        ...
                        writeSubtitle.setText(seqLists.get(seqRecylcerAdapter.selected_position).text);
            ...
            case R.id.eraseBtn :
                ...
                if(selectPos !=-1)
                {
                    if(seqLists.get(selectPos).text!=null)
                    seqLists.get(selectPos).text=null;
                    seqRecylcerAdapter.notifyItemChanged(selectPos);
                }
                break;
            case R.id.inputSubBtn :
                if(seqRecylcerAdapter.selected_position!=-1) {
                    seqLists.get(seqRecylcerAdapter.selected_position).text = writeSubtitle.getText().toString();
                    seqRecylcerAdapter.notifyItemChanged(seqRecylcerAdapter.selected_position);
                }
                writeSubtitle.setText(null);
                writeSubtitle.clearFocus();
                mgr.hideSoftInputFromWindow(writeSubtitle.getWindowToken(), 0);
                break;
            case R.id.uploadBtn :
